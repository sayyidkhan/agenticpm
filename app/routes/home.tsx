...
function AppLayout() {
  const navigate = useNavigate();
  const { parsed, error, isSaving, hasUnsavedChanges, canUndo, undo, saveAll, activeFileName, activeProjectName, setCurrentSprint, isLocked, isReadOnly, lockedByUser, sessionId } = useProject();
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
          ...
          {isReadOnly && (
            <div className="border-t border-yellow-500/50 bg-yellow-500/10 px-4 py-2 text-sm text-yellow-700 dark:text-yellow-400 flex items-center gap-2">
              ...
              <span><strong>Read-only mode</strong> — This project is being edited by {lockedByUser ? <strong>{lockedByUser}</strong> : "another user"}. You can view but not edit.</span>
            </div>
          )}
